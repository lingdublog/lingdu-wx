...
var componentStyleHtml = '.component_mask{' +
...
ToastClass.prototype.show = function (options) {
  ...
  var _this = this;
  var template = options.template || '正在加载……';
  var duration = options.duration;
  var callback = options.callback;
  var loading = options.loading;
  var toast = document.createElement ('div');
  var innerHTMLArr = ['<div class="component_mask"><div class="component_toast">'];
  if (loading) {
  ...
  innerHTMLArr.push('<div style="font-size: 16px;margin:10px 0;">' + template + '</div></div></div>');
  toast.innerHTML = innerHTMLArr.join ('');
  document.body.appendChild (toast);

  _this.instance = toast;
...
ConfirmClass.prototype.show = function (options) {
  ...
  var _this = this;
  var defaultBtns = [{text: '确定', class: 'confirm'}, {text: '取消', class: 'cancel'}];
  ...
  this.btns.forEach(function (v, i){
    btnHtml += '<a class="component_btn '+ (v.class||'') +'" style="cursor: pointer;" onclick="ConfirmClass.btnClick('+ i +')">'+ (v.text||'确定') +'</a>'
  });
...
Validator.prototype.doValidate = function(eleName, validateObj){
  var self = this;
  var value = self.parent.querySelector('[name=' + eleName + ']').value.replace(/(^\s*)|(\s*$)/g, '');
  // 判断必填选项
  ...
  if (validateObj.equal) {
    var equalValue = self.parent.querySelector('[name=' + validateObj.equal.name + ']').value.replace(/(^\s*)|(\s*$)/g, '');
    if (value === equalValue) {
      self.setTrue(eleName);
    }else{
  ...
  if(validateObj.scopes && !isNaN(value)){
    for(var j in validateObj.scopes){
  ...
  if(validateObj.patterns){
    for(var i in validateObj.patterns){
